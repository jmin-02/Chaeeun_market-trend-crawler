...
Bloter is a Korean tech news site covering AI, startups, and technology trends.
"""
...
class BloterCrawler(BaseCrawler):
    """Crawler for Bloter (블로터) - https://www.bloter.net"""

    BASE_URL = "https://www.bloter.net"

    def extract_articles(self, html: str, source: str, language: str = "ko") -> list[Article]:
        """Extract articles from Bloter with enhanced error handling.
        ...
        for idx, item in enumerate(article_items):
            try:
                ...
                # Determine category from URL tags
    ...
    def _extract_title(self, item, source: str, index: int) -> Optional[str]:
        ...
        try:
            ...
            if not title or len(title) < 3:
                error_context = create_error_context(
                    source=source, article_index=index, field_name="title", field_value=title
                )
                logger.debug(f"Invalid title at index {index}: '{title}'")
                return None
    ...
    def _extract_content(self, item, source: str, index: int, default_title: str) -> str:
        ...
        Args:
            ...
            source: Source name
            index: Article index
            default_title: Fallback content if extraction fails
